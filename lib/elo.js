...
// label from their numeric Elo rating.
export const SKILL_CATEGORIES = [
  { id: 'D', label: 'Кат. D', sub: 'Новачки', range: [800, 1000], color: '#5DCAA5' },
  { id: 'C', label: 'Кат. C', sub: 'Любителі', range: [1000, 1200], color: '#1D9E75' },
  { id: 'B', label: 'Кат. B', sub: 'Досвідчені', range: [1200, 1500], color: '#0F6E56' },
  { id: 'A', label: 'Кат. A', sub: 'Просунуті', range: [1500, 1800], color: '#085041' },
  { id: 'Open', label: 'Open', sub: 'Про', range: [1800, 2200], color: '#04342C' },
];

// Default starting Elo when an admin approves a new player into a category.
...
export function categoryForElo(elo) {
  ...
  if (elo >= 1800) return SKILL_CATEGORIES[4];
  if (elo >= 1500) return SKILL_CATEGORIES[3];
  if (elo >= 1200) return SKILL_CATEGORIES[2];
  if (elo >= 1000) return SKILL_CATEGORIES[1];
