...
export function computeStandings(players, matches) {
  ...
  matches
    ...
    .forEach((m) => {
      ...
      const deltaForA = eloDelta(teamAAvgElo, teamBAvgElo, teamAWon ? 1 : 0);
      const deltaForB = eloDelta(teamBAvgElo, teamAAvgElo, teamAWon ? 0 : 1);

      m.team_a_players.forEach((pid) => {
        if (!stats[pid]) return;
        stats[pid].gamesFor += m.score_a;
        stats[pid].gamesAgainst += m.score_b;
        stats[pid].played += 1;
        stats[pid].eloDelta += deltaForA;
        if (teamAWon) stats[pid].wins += 1;
      });

      m.team_b_players.forEach((pid) => {
        if (!stats[pid]) return;
        stats[pid].gamesFor += m.score_b;
        stats[pid].gamesAgainst += m.score_a;
        stats[pid].played += 1;
