...
export interface KingRoundResult {
  groups: (string | null)[][];
  matches: Match[];
}
 ...
 *
 * @param playerIds - players in the intended group order; length must
 *   be divisible by 4 (sequential slices become groups)
 * @param courts - court numbers in use
 * @param roundNo - 1-based King round; encoded as stage 'kr<N>'
 */
  ...
  groups.forEach((group, groupIndex) => {
    ...
    GROUP4_SCHEDULE.forEach((g) => {
      matches.push({
        ...
        court,
        team_a_players: g.teamA.map((slot) => group[slot]) as string[],
        team_b_players: g.teamB.map((slot) => group[slot]) as string[],
        played: false,
      });
    });
  });
...
export function buildKingRound1(playerIds: string[], courts: number[]): KingRoundResult {
 ...
 */
export function kingRoundSizes(n: number): number[] {
  const sizes = [n];
 ...
 */
export function buildKingPlaceholders(n: number, courts: number[]): Match[] {
  const rows: Match[] = [];
 ...
 */
export function rankGroupDetailed(groupPlayerIds: string[], groupMatches: Match[]): GroupStanding[] {
  const stats: Record<string, GroupStanding> = Object.fromEntries(
    groupPlayerIds.map((id) => [id, { id, wins: 0, diff: 0 }])
  );
...
export function rankGroup(groupPlayerIds: string[], groupMatches: Match[]): string[] {
 ...
 *
 * @returns player ids, flattened in fresh-group order
 */
export function kingAdvancers(rankedGroups: GroupStanding[][]): string[] {
