...
export function seedCapacity(category) {
 ...
 *
 * @param {Array} rows - roster rows carrying `slotIndex` (number|null),
 *   already in fallback order (see seedRoster / bySeed): the order the
 *   unseeded ones should take.
 * @param {number|null} capacity - grid size, or null for a dense list.
 * @returns {Array} array of length max(capacity, rows.length): the row
 *   sitting at each place, or null for an empty one (a bye).
 */
export function placeIntoSlots(rows, capacity) {
  ...
  for (const row of rows) {
    ...
    if (Number.isInteger(i) && i >= 0 && i < size && slots[i] === null) slots[i] = row;
    else rest.push(row);
  }
  let cursor = 0;
