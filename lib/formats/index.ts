...
export type BracketSystemId =
  | 'double_elimination'
  | 'groups_crosses_1_2'
  | 'groups_top1_bye_top23_crosses';

export interface BracketSystem {
  id: BracketSystemId;
  label: string;
  shortLabel: string;
  /** Admin picks the bracket size (double-elim only). */
  sizeChoice: boolean;
...
export function getBracketSystem(id: string | null | undefined): BracketSystem | null {
...
export function participantOptionsFor(bracketSystemId: string | null | undefined): number[] {
...
export function defaultParticipantsFor(bracketSystemId: string | null | undefined): number | null {
  const b = getBracketSystem(bracketSystemId);
  if (!b) return null;
  return b.sizeChoice ? b.participantOptions[0] : b.cap;
}

export type FormatKindId = 'americanka' | 'single_gender' | 'mix' | 'king_of_beach';

export interface FormatKind {
  kind: FormatKindId;
  displayName: string;
  description: string;
  registrationType: 'solo' | 'pair' | 'mix_pair';
  hasGender: boolean;
  fixedParticipants: number | null;
  participantOptions: number[] | null;
  countsPairs: boolean;
  needsBracketSystem: boolean;
  scoring: 'sum31' | 'first_to';
  maxSets: number;
}
...
export function getFormat(kind: string | null | undefined): FormatKind | null {
  return FORMAT_KINDS[kind as FormatKindId] || null;
}

export function listFormats(): FormatKind[] {
