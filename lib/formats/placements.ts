...
import { rankGroupDetailed, type GroupStanding } from './kingOfBeach';
import { computeStandings } from '../tournamentEngine';
import type { Match, Player, PlacementRow, Team } from '../types';

const winnerLoser = (m: Match): { w: string[]; l: string[] } => {
...
const teamKey = (ids: (string | null | undefined)[] | null | undefined): string =>
  [...(ids || [])].filter(Boolean).map(String).sort().join('|');
 ...
 *
 * @returns ascending by place
 */
export function placementsFor({ matches, teams, players }: PlacementsForArgs): PlacementRow[] {
  const ms = matches || [];
  ...
  // placement, 1..N with no ties.
  if (!ms.some((m) => m.stage)) {
    if (ms.some((m) => !m.played)) return [];
    return computeStandings(players || [], ms).map((s, i) => ({
      place: i + 1,
...
export function computePlaces(matches: Match[], teams: Team[]): PlacementRow[] {
...
export function kingResults(matches: Match[]): PlacementRow[] {
...
export function deResults(matches: Match[]): PlacementRow[] {
  const out: PlacementRow[] = [];
  const legacy = matches.some((m) => m.stage === 'gf');
  const playedOut: [string, number, number][] = legacy
    ? [['gf', 1, 2]]
    : [
        ['final', 1, 2],
        ['p3_4', 3, 4],
  ...
  const lbRounds = [
    ...new Set(
      matches
        .filter((m) => /^lb\d+$/.test(m.stage || ''))
        .map((m) => Number((m.stage ?? '').slice(2)))
    ),
  ].sort((a, b) => b - a);
