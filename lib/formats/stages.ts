...
function placeLabel(stage: string): string | null {
...
function bracketOf(stage: string): 'main' | 'cons' | 'single' {
...
export function stageWeight(stage: string): number {
  ...
  // The bronze match is numbered just BEFORE the final on the paper
  // sheet (гра 29 vs 30 in the 16-pair bracket), so it sorts there too;
  // the remaining placement games follow everything else.
  if (stage === 'p3_4') return 125;
...
// Placement games that share the trailing «Матчі за місця» column of the
// bracket views. The bronze match is excluded — it has its own column
// right before the final, like on the paper sheet.
export function isSharedPlaceStage(stage: string | null | undefined): boolean {
  return /^p\d+_\d+$/.test(stage || '') && stage !== 'p3_4';
}

export function stageLabel(stage: string): string {
  const kr = /^kr(\d+)$/.exec(stage);
  if (kr) return `Раунд ${kr[1]}`;
  // Double elimination: the club calls the two brackets Winners and
  // Losers, not «верхня/нижня».
  const de = /^(wb|lb)(\d+)$/.exec(stage);
  if (de) return `${de[1] === 'wb' ? 'Winners' : 'Losers'} · Раунд ${de[2]}`;
...
export function groupTitle(groupIndex: number): string {
