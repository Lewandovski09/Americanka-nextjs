 ...
 *
 * @param teams - [{ id, seed }] best-first
 */
export function splitIntoGroups<T>(teams: T[], groupSize = 4): T[][] {
 ...
 */
export function roundRobinPairings(teamCount: number): RoundRobinRound[] {
  const idx: (number | null)[] = [...Array(teamCount).keys()];
  ...
  for (let r = 0; r < n - 1; r++) {
    ...
    // rotate, keeping the first element fixed. Array always has >= 2
    // elements here (teamCount >= 2 going in), so pop() never sees an
    // empty array — the `!` just satisfies TS's general pop() signature.
...
export function teamKey(players: (string | null | undefined)[] | null | undefined): string {
  return [...(players || [])].filter(Boolean).map(String).sort().join('|');
}

export function stageForCount(n: number): string {
  if (n === 2) return 'final';
 ...
 *   array of team objects — so a winner meets a runner-up of another
 *   group first once fed through interleavePlaces.
 */
export function computeGroupRanking(teams: SeedTeam[], groupMatches: Match[]): SeedTeam[][] {
  const keyToTeam = new Map(teams.map((t) => [teamKey(t.players), t]));
  const groups = new Map<number, Map<string, GroupStat>>(); // groupIndex -> Map(teamId -> stat)
  ...
  groupMatches
    ...
    .forEach((m) => {
      ...
      const sides: [SeedTeam | undefined, number, number, boolean][] = [
        [tA, ptsA, ptsB, aWon],
        [tB, ptsB, ptsA, !aWon],
      ];
      for (const [team, gf, ga, won] of sides) {
        if (!team) continue;
...
export function interleavePlaces(rankedByGroup: SeedTeam[][], ranks: number[]): SeedTeam[] {
  const out: SeedTeam[] = [];
...
export function computeGroupQualifiers(teams: SeedTeam[], groupMatches: Match[], topN = 2): SeedTeam[] {
...
export function currentFrontier(koMatches: Match[]): Frontier | null {
 ...
 */
export function buildKnockoutRound(orderedTeams: SeedTeam[], courts: number[]): Match[] {
...
/** A team as it flows out of a completed round — the tournament_teams
 * row when it's known, or a bare id-less stand-in when the round was
 * seeded from a bracket the team map doesn't cover. */
interface Advancer {
  id: string | null;
  players: string[];
  [key: string]: unknown;
 ...
 */
export function knockoutSurvivors(teams: SeedTeam[], roundMatches: Match[]): Advancer[] {
  const keyToTeam = new Map(teams.map((t) => [teamKey(t.players), t]));
  return [...roundMatches]
...
export function buildGroupStage(teams: SeedTeam[], courts: number[], groupSize = 4): Match[] {
...
export function buildTwoGroupStage(teams: SeedTeam[], courts: number[]): Match[] {
...
export function buildFourGroupStage(teams: SeedTeam[], courts: number[]): Match[] {
  return buildGroupStage(teams, courts, 4);
}

interface PlayoffPointer {
  id: string;
  slot: 'a' | 'b';
}

interface PlayoffNode {
  id: string;
  stage: string;
  round: number;
  a: SeedTeam | null;
  b: SeedTeam | null;
  winnerTo: PlayoffPointer | null;
  loserTo: PlayoffPointer | null;
  isFinal: boolean;
  walkover: boolean;
}

// Serialize playoff nodes into match rows the score route can propagate.
function serializePlayoff(nodes: PlayoffNode[], courts: number[]): Match[] {
...
const mkNode = (stage: string, round: number, extra: Partial<PlayoffNode> = {}): PlayoffNode => ({
  id: randomUUID(),
  stage,
  round,
  a: null,
...
function placementNodes(gA: SeedTeam[], gB: SeedTeam[], startIndex: number): PlayoffNode[] {
  const nodes: PlayoffNode[] = [];
...
export function buildCrossesPlayoff(rankedGroups: SeedTeam[][], courts: number[]): Match[] {
...
export function buildByeCrossesPlayoff(rankedGroups: SeedTeam[][], courts: number[]): Match[] {
...
export function computePlacements(matches: Match[] | null | undefined): BracketPlacementRow[] {
  const places: BracketPlacementRow[] = [];
