...
/**
 * @param {Array} teams - the seed column, best-first: [{ id, players:[p1,p2] }]
...
export function buildDoubleElimination(teams, courts, bracketSize) {
  ...
  wb.slice(1).forEach((wbRound, t) => {
    ...
    const survivorSlot = last ? 'b' : 'a';
    const dropSlot = last ? 'a' : 'b';
  ...
  // left empty on «Посів».
  const at = (seed) => (seed <= P && teams[seed - 1] ? teams[seed - 1] : BYE);
  wb[0].forEach((m, i) => {
    m.a = at(order[2 * i]);
    m.b = at(order[2 * i + 1]);
  });
