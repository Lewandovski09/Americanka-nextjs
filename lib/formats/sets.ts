...
// single set.
//
// No imports from other lib/ modules so it is safe in both server
// routes and client components.

import type { MatchSets, SetScore } from '../types';

/** The filled sets of a match, in order: [[a,b], ...] (0–3 entries). */
export function setsOf(m: MatchSets): SetScore[] {
  return [m.set1, m.set2, m.set3].filter(
    (s): s is SetScore => Array.isArray(s) && s.length === 2
  );
}
 ...
 */
export function aggregateScore(m: MatchSets): SetScore | null {
  const sets = setsOf(m);
...
export function teamAWon(m: MatchSets): boolean {
  const agg = aggregateScore(m);
  return agg ? agg[0] >= agg[1] : false;
}

/** Total points [for_a, for_b] summed across every set. */
export function pointsTotals(m: MatchSets): SetScore {
  return setsOf(m).reduce<SetScore>((acc, s) => [acc[0] + s[0], acc[1] + s[1]], [0, 0]);
}

/** Team A's points differential summed across every set. */
export function pointsDiffA(m: MatchSets): number {
  const [a, b] = pointsTotals(m);
  return a - b;
}

/** Display label: '21:15' or '2:1 (21:15, 15:21, 15:10)'. null if unplayed. */
export function scoreLabel(m: MatchSets): string | null {
  const agg = aggregateScore(m);
  if (!agg) return null;
  const sets = setsOf(m);
